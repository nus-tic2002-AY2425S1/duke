...
/**
 * <h1>Add Command</h1>
 * The AddCommandTest program implements an application that
...
public class AddCommandTest extends Command {
    ...
    public AddCommandTest(String action, String instruction) {
    ...
    public void execute(TaskList tasks, Ui ui, Storage storage) throws IOException, NoArgsException {
        ...
        switch (getAction()) {
            case "todo":
                tasks.add(new TodoTest(getInstruction()));
            ...
            case "deadline":
                ...
                try {
                    tasks.add(new DeadlineTest(deadlineInstruction.get(0), LocalDate.parse(deadlineInstruction.get(deadlineInstruction.size()-1), buildFormatter())));
                    System.out.println("Got it, I've added this task: \n" + tasks.getLast().print());
                    ui.showSize(tasks.size());
            ...
            case "event":
                ...
                tasks.add(new EventTest(eventInstruction.get(0), eventInstruction2.get(0), eventInstruction2.get(eventInstruction.size()-1)));
                System.out.println("Got it, I've added this task: \n" + tasks.getLast().print());
                ui.showSize(tasks.size());
                break;
        }
